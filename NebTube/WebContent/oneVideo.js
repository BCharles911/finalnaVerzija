	...
	$(document).ready(function(e) {
		...
		$.get('OneVideoServlet',{videoID:videoID},function(data) {
			...
			if(data.userStatus == 'Authenticated' && data.loggedInUser.role != 'ADMINISTRATOR' && data.loggedInUser.username != data.video.ownerUsername) {
				commentSection.append('<p style="text-align:center;">Comments are not allowed for this video');
				$('#commentInput').hide();
				$('#commentSubmit').hide();
				$('#sortComments').hide();
			} else if(data.loggedInUser.role == 'ADMINISTRATOR' || data.loggedInUser.username == data.video.ownerUsername) {
