...
public class UploadVideoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			for(FileItem item : items) {
				...
				if(!item.isFormField()) {					
					...
					if(item.getContentType().toString().split("/")[0].toString().equals("image")) {
						...
						//System.out.println("uporedjivao : " + item.getContentType().toString().split("/")[0].toString());
					}

				}else {
					//System.out.println(("Polje: " + item.getFieldName() + " Vrednost: " + item.getString()));
		...
		try {
			...
			//System.out.println("NIJE DOBAR FORMAT EMBEDURLA");
		}
